...
public float szer_przycisku = 250;
	...
	void OnGUI()
	{
            ...
            foreach (Gracz graczzzzz in graczeeeInJson.graczeee)
            {
                ...
                if(spr==null)
                {
                    StartCoroutine(DownloadFile(graczzzzz.obrazek));
     ...
     IEnumerator DownloadFile(string nazwa) {
        var uwr = new UnityWebRequest("ddd.ugu.pl/"+nazwa, UnityWebRequest.kHttpVerbGET);
